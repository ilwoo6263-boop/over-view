...
function getTimeContext(date = new Date()) { const hour = date.getHours(); if (hour >= 5 && hour < 8) return 'sunrise'; if (hour >= 18 && hour < 20) return 'sunset'; if (hour >= 20 || hour < 5) return 'night'; return 'day'; }
function decodePolyline(encoded) {
  let index = 0, lat = 0, lng = 0; const path = [];
...
function routeSegmentsFromPath(path, route) {
  const total = Math.max(path.length - 1, 1);
  return path.map((point, index) => ({ id: `google-${index}`, ...point, heading: 0, transportMode: route.mode, etaMinutes: Math.max(1, Math.round((route.durationSeconds / 60) * index / total)) }));
}
function renderPoints(points) {
  resultCount.textContent = `${points.length} discoveries`;
  pointsEl.innerHTML = points.map((point, index) => `<article class="point ${index === 0 ? 'point-featured' : ''}"><div class="point-top"><div class="point-icon">${pointIcons[point.category] || '👀'}</div><span>${point.etaMinutes ?? '—'}분 후</span></div><h3>${point.title}</h3><p>${point.description}</p><div class="point-footer"><span class="tag">${point.tags.join(' · ')}</span><span>${point.viewingSide === 'left' ? '← 왼쪽' : point.viewingSide === 'right' ? '오른쪽 →' : '↑ 정면'}</span></div></article>`).join('');
}
...
async function initMap() {
  ...
  try { await loadGoogleMaps(key); map = createMap(mapEl); mapStatus.textContent = 'Google Maps 연결됨'; }
  catch (error) { mapStatus.textContent = 'Google Maps API 키를 설정하면 실제 지도를 사용할 수 있습니다.'; console.warn(error.message); }
}
async function runRoute() {
  ...
  try {
    ...
    const path = decodePolyline(route.encodedPolyline); route.segments = routeSegmentsFromPath(path, route);
    ...
    if (map) {
      ...
      ranked.slice(0, 8).forEach(point => { if (Number.isFinite(point.lat) && Number.isFinite(point.lng)) markers.push(addViewMarker(map, point)); });
    ...
    console.error(error); routeSummary.textContent = error.message.includes('API key') ? 'Google Maps API 키를 먼저 설정해주세요.' : `경로를 계산하지 못했습니다: ${error.message}`; pointsEl.innerHTML = '<p>Google Maps 경로를 불러오지 못했습니다.</p>'; moment.hidden = true;
